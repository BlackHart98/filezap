...
extern int fz_query_required_chunk_list(context_t *context, fz_ctx_t *ctx, fz_file_manifest_t *mnfst, fz_chunk_t **chunk_buffer, size_t *nchunk, struct missing_chunks_map_s *missing_chunks){
    ...
    /* Pre-allocate at chunk_seq_len, resize if needed */
    size_t max_alloc = mnfst->chunk_seq.chunk_seq_len;
    slice_t buffer_slice = arena_allocator_alloc((&context->temp_allocator), fz_chunk_t, max_alloc);
    if (NULL == buffer_slice.ptr) RETURN_DEFER(0);
    fz_chunk_t *buffer = (fz_chunk_t *)buffer_slice.ptr;
    ...
    while (SQLITE_ROW == (ret = sqlite3_step(stmt))){
        ...
        if (local_nchunk >= max_alloc) {
            max_alloc *= 2;
            buffer_slice = arena_allocator_resize(&(context->temp_allocator), fz_chunk_t, buffer_slice, max_alloc);
            if (NULL == buffer_slice.ptr) RETURN_DEFER(0);
            buffer = (fz_chunk_t *)buffer_slice.ptr;
        }

        size_t path_len = strlen(file_path);
        slice_t path_slice = arena_allocator_alloc(&(context->temp_allocator), char, path_len + 1);
        if (NULL == path_slice.ptr) RETURN_DEFER(0);
        memcpy(path_slice.ptr, file_path, path_len);
        ...
        buffer[local_nchunk].src_file_path   = (char *)path_slice.ptr;
        local_nchunk++;
    }
