...
int main(int argc, char *argv[]){
    ...
    if (-1 == child_process){
        ...
        if (!fz_channel_init_v2(&(context.allocator), &snd_channel, FZ_TCP_SOCKET, FZ_SENDER_MODE, NULL)){
        ...
        if (!fz_channel_init_v2(&(context.allocator), &recv_channel, FZ_TCP_SOCKET, FZ_RECEIVER_MODE, NULL)){
            fz_log(FZ_ERROR, "%s: Failed to initialize file zap sender channel", __func__);
            RETURN_DEFER(1);
        }
